...
import { Category, type Report, type FieldPrompt, type SeverityLevel } from '../types';
import { useTranslations } from '../i18n';
import { CheckCircle, Zap, Target, Camera, Loader, Sparkles, Activity, ShieldCheck } from './icons';
...
const TutorialMissionView: React.FC<TutorialMissionViewProps> = ({ onComplete, onSkip }) => {
      ...
      case 'photo':
        return (
          <div key={prompt.id} className="space-y-4" {...containerProps}>
            <label className="text-[10px] font-black text-cyan-500 uppercase tracking-widest">{prompt.promptText}</label>
            <div
              className="aspect-video bg-zinc-950 border-2 border-dashed border-zinc-800 rounded-[2.5rem] flex flex-col items-center justify-center p-8 text-center cursor-pointer hover:border-cyan-500 transition-all group shadow-inner"
              onClick={() => {
  ...
  return (
    <div className="bg-black min-h-screen text-white font-mono p-4 md:p-8 animate-fade-in">
      {/* DO NOT SHOW ON LOAD. ONLY SHOW WHEN showTutorial IS TRUE. */}
      {showTutorial && overlayStep && (
        <TutorialOverlay
          enabled={true}
          step={overlayStep}
      ...
      <div className="max-w-6xl mx-auto space-y-12 pb-32">
        <header className="flex justify-between items-center bg-zinc-900/50 p-8 rounded-[3rem] border border-zinc-800 shadow-2xl relative overflow-hidden">
          ...
          {/* OPTIONAL: Button to manually open tutorial */}
          <div className="hidden md:flex items-center space-x-3">
            <button
              onClick={() => setShowTutorial(true)}
              className="bg-zinc-900 hover:bg-zinc-800 text-zinc-300 px-6 py-3 rounded-xl border border-zinc-800 text-[10px] font-black uppercase tracking-widest transition-all"
            >
              Help_Tutorial
            </button>
          </div>
              ...
              {tutorialReportForDutyMission.actions.map((act, idx) => {
                ...
                return (
                  <div key={act.id} className={`flex items-center space-x-6 relative z-10 transition-all duration-500 ${idx > activeActionIndex ? 'opacity-20 grayscale' : 'opacity-100'}`}>
                    <div className={`w-10 h-10 rounded-xl border-2 flex items-center justify-center transition-all ${isPast ? 'bg-emerald-500 border-emerald-400 text-black shadow-[0_0_10px_rgba(16,185,129,0.5)]' : isCurrent ? 'bg-cyan-500 border-cyan-400 text-white shadow-[0_0_15px_cyan]' : 'bg-zinc-950 border-zinc-800 text-zinc-700'}`}>
              ...
              <div className="p-8 md:p-12 space-y-12">
                ...
                <div className="pt-10 border-t border-zinc-800">
                  <button
                    ...
                    className={`w-full py-8 rounded-[2rem] font-black uppercase tracking-[0.5em] text-xs transition-all shadow-4xl active:scale-[0.98] flex items-center justify-center space-x-6 border-b-8 ${canAdvance ? 'bg-white text-black border-zinc-300 hover:bg-cyan-50' : 'bg-zinc-900 text-zinc-700 border-zinc-950 opacity-40 cursor-not-allowed'}`}
                  >
