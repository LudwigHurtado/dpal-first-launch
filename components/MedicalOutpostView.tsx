...
const MedicalOutpostView: React.FC<MedicalOutpostViewProps> = ({ onReturn, hero, records, setRecords }) => {
  ...
  // REMOVED: any cloud drive fields. This prevents any Google Drive prompts.
  ...
  const handleOpenEdit = (rec: HealthRecord) => {
    setFormRecord({
      ...
      relationship: (rec as any).relationship || 'Family',
      bloodType: (rec as any).bloodType || '',
      dob: (rec as any).dob || '',
      allergies: Array.isArray((rec as any).allergies) ? (rec as any).allergies.join(', ') : '',
      medications: Array.isArray((rec as any).medications) ? (rec as any).medications.join(', ') : '',
      emergencyContact: (rec as any).emergencyContact || '',
      criticalNotes: (rec as any).criticalNotes || '',
  ...
  const normalizeList = (s: string) => s.split(',').map(x => x.trim()).filter(Boolean);

  const handleCreateOrUpdate = () => {
    if (!formRecord.ownerName.trim()) return;
        ...
        prev.map(r =>
          r.id === editingRecordId
            ? ({
                ...
                dob: formRecord.dob,
                allergies: normalizeList(formRecord.allergies),
                medications: normalizeList(formRecord.medications),
  ...
  const loadDemoData = () => {
    const demo: HealthRecord = {
      ...
      isCloudSynced: false,
      timestamp: Date.now(),
      criticalNotes: 'Demo record. Replace with real data.',
    } as any;
  ...
  // REMOVED: any Drive URI from the QR value.
  const getQrValue = (cat: FolderCategory | 'PROFILE') => {
    if (!activeRecord) return '';
    const folderPrefix = cat !== 'PROFILE' ? `:${cat}` : '';
    return `QRATE_MED:${activeRecord.id}${folderPrefix}:VAULT:LOCAL_ONLY`;
  };

  const qrImageUrl = (val: string) =>
    `https://api.qrserver.com/v1/create-qr-code/?size=600x600&data=${encodeURIComponent(val)}&bgcolor=ffffff&color=0f172a&margin=10`;
          ...
          <div className="flex bg-black/40 p-1.5 rounded-2xl border border-zinc-800">
            <button
              ...
              className={`px-6 py-2 rounded-xl text-[10px] font-black transition-all ${
                language === 'EN' ? 'bg-zinc-800 text-cyan-400' : 'text-zinc-600'
              }`}
            >
              EN
            </button>
            <button
              ...
              className={`px-6 py-2 rounded-xl text-[10px] font-black transition-all ${
                language === 'ES' ? 'bg-zinc-800 text-cyan-400' : 'text-zinc-600'
              }`}
            >
              ES
            </button>
            ...
            {records.length === 0 ? (
              ...
              records.map(rec => (
                <button
                  ...
                  className={`w-full text-left p-6 rounded-[2rem] border-2 transition-all flex items-center space-x-4 ${
                    editingRecordId === rec.id
                      ? 'bg-zinc-900 border-cyan-600 shadow-xl scale-[1.02]'
                ...
                >
                  <div className="w-14 h-14 rounded-2xl overflow-hidden border-2 border-zinc-800 bg-zinc-900 flex-shrink-0">
                    <img
                      src={(rec as any).profilePicture || `https://api.dicebear.com/7.x/avataaars/svg?seed=${rec.ownerName}`}
                      alt="P"
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[8px] text-zinc-600 font-black uppercase tracking-widest mt-0.5">{(rec as any).relationship}</p>
                  </div>
                </button>
              ))
            )}
            ...
            <div className="max-w-4xl mx-auto space-y-12 animate-fade-in no-print">
              <header className="flex justify-between items-start">
                <div>
                  <h2 className="text-5xl font-black tracking-tighter text-white uppercase leading-none">
                    {editingRecordId ? 'Update Shard' : 'Initialize Shard'}
                  </h2>
                  <p className="text-zinc-500 font-bold mt-4 tracking-widest uppercase text-[10px]">Configure identity</p>
                </div>
                <button onClick={() => setIsCreating(false)} className="p-4 bg-zinc-900 rounded-2xl hover:bg-zinc-800 transition-all">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
                <div className="md:col-span-4 space-y-6">
                  ...
                  <p className="text-[8px] text-center text-zinc-600 font-bold uppercase tracking-widest italic">Verification photo recommended</p>
                </div>
                ...
                <div className="md:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <input
                      value={formRecord.ownerName}
                      onChange={e => setFormRecord({ ...formRecord, ownerName: e.target.value })}
                      className="w-full bg-zinc-950 border-2 border-zinc-800 rounded-2xl p-5 outline-none focus:border-cyan-600 transition-all shadow-inner font-bold text-white uppercase text-sm"
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="date"
                      value={formRecord.dob}
                      onChange={e => setFormRecord({ ...formRecord, dob: e.target.value })}
                      className="w-full bg-zinc-950 border-2 border-zinc-800 rounded-2xl p-5 outline-none focus:border-cyan-600 transition-all shadow-inner font-bold text-zinc-400"
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      value={formRecord.bloodType}
                      onChange={e => setFormRecord({ ...formRecord, bloodType: e.target.value })}
                      className="w-full bg-zinc-950 border-2 border-zinc-800 rounded-2xl p-5 outline-none focus:border-cyan-600 transition-all shadow-inner font-bold text-white uppercase text-sm"
                      placeholder="O+, A-, etc."
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      value={formRecord.emergencyContact}
                      onChange={e => setFormRecord({ ...formRecord, emergencyContact: e.target.value })}
                      className="w-full bg-zinc-950 border-2 border-zinc-800 rounded-2xl p-5 outline-none focus:border-cyan-600 transition-all shadow-inner font-bold text-white text-sm"
                      placeholder="Node Phone"
                    />
                  </div>

                  <div className="md:col-span-2 space-y-2">
                    <label className="text-[10px] font-black uppercase text-zinc-600 tracking-widest ml-4">Critical Notes</label>
                    <textarea
                      value={formRecord.criticalNotes}
                      onChange={e => setFormRecord({ ...formRecord, criticalNotes: e.target.value })}
                      className="w-full bg-zinc-950 border-2 border-zinc-800 rounded-2xl p-5 outline-none focus:border-cyan-600 transition-all shadow-inner font-bold text-white text-sm min-h-[120px]"
                      placeholder="Key conditions, warnings, and notes."
                    />
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-black py-8 rounded-[2.5rem] uppercase tracking-[0.4em] text-xs shadow-2xl active:scale-[0.98] transition-all"
              >
            ...
            <div className="animate-fade-in space-y-16 pb-16">
              <div className="no-print flex flex-col md:flex-row justify-between items-start md:items-center gap-8 border-b border-zinc-800 pb-12">
                <div className="flex items-center space-x-10">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-8 mt-6">
                      <span className="bg-cyan-950/40 text-cyan-400 px-6 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-cyan-900/50">
                        {(activeRecord as any).relationship}
                      </span>
                      <div className="flex items-center space-x-3">
                        <Heart className="w-5 h-5 text-rose-500" />
                        <span className="text-zinc-500 font-black text-[10px] uppercase tracking-widest">
                          BLOOD: {(activeRecord as any).bloodType || 'N/A'}
                        </span>
                      </div>
                      <div className="flex items-center space-x-3">
                        <Monitor className="w-5 h-5 text-cyan-500" />
                        <span className="text-zinc-500 font-black text-[10px] uppercase tracking-widest">
                          DOB: {(activeRecord as any).dob || 'N/A'}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex items-center space-x-4">
                  <button onClick={() => window.print()} className="p-4 bg-zinc-900 hover:bg-zinc-800 text-zinc-500 hover:text-white rounded-2xl transition-all border border-zinc-800">
                    <Printer className="w-6 h-6" />
                  </button>
                  <button onClick={() => handleOpenEdit(activeRecord)} className="p-4 bg-zinc-900 hover:bg-zinc-800 text-zinc-500 hover:text-white rounded-2xl transition-all border border-zinc-800">
                    <Pencil className="w-6 h-6" />
                  </button>
                  <button onClick={() => handleDelete(activeRecord.id)} className="p-4 bg-zinc-900 hover:bg-rose-950 text-zinc-500 hover:text-rose-500 rounded-2xl transition-all border border-zinc-800">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                <div className="lg:col-span-7 space-y-16">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      {MEDICAL_FOLDERS.map(folder => (
                        <button
                          ...
                          className={`p-8 rounded-[2.5rem] border-2 text-left transition-all flex flex-col items-start space-y-6 relative group ${
                            activeFolder === folder.id
                              ? 'bg-cyan-600 text-white border-cyan-500 shadow-2xl scale-[1.03]'
                              : 'bg-zinc-950 border-zinc-800 hover:border-zinc-700 shadow-sm'
                          }`}
                        >
                          <div className={`p-4 rounded-2xl transition-colors ${activeFolder === folder.id ? 'bg-white/20 text-white' : 'bg-zinc-900 text-zinc-600 border border-zinc-800'}`}>
                            {folder.icon}
                          </div>
                  ...
                  <section className="space-y-8">
                    <h3 className="text-[10px] font-black uppercase text-zinc-600 tracking-[0.4em]">
                      Local Vault Mode
                    </h3>
                    <div className="bg-zinc-950 rounded-[3rem] p-10 text-white shadow-2xl border border-zinc-800">
                      <p className="text-[9px] font-black text-cyan-500 uppercase tracking-widest mb-6">Storage</p>
                      <p className="text-sm font-mono text-zinc-400 leading-relaxed">
                        This screen uses local storage only. No Google Drive. No sign in. No external vault links.
                      </p>
                    </div>
                  </section>
                </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-20 space-y-16">
              ...
              <div className="space-y-6">
                ...
                <p className="text-xs text-zinc-600 font-black uppercase tracking-[0.5em] max-w-2xl mx-auto leading-relaxed">
                  INITIALIZE SECURE CLINICAL SHARDS FOR FAMILY MEMBERS.
                </p>
              </div>
              <div className="flex flex-wrap justify-center gap-8">
                <button onClick={() => setIsCreating(true)} className="bg-cyan-600 hover:bg-cyan-500 text-white font-black py-8 px-20 rounded-[3rem] uppercase tracking-[0.4em] text-[10px] shadow-2xl active:scale-95 transition-all">
                  Materialize Shard
                </button>
                <button onClick={loadDemoData} className="bg-zinc-950 border-2 border-zinc-800 text-zinc-500 font-black py-8 px-16 rounded-[3rem] uppercase tracking-[0.4em] text-[10px] transition-all hover:bg-zinc-900 active:scale-95">
                  Sync Sample Node
                </button>
              </div>
            </div>
